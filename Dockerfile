...
# Configure stylesheet(s)
# Note repositories with style changes, should be given new tags and updated below to ensure docker picks up changes.
RUN mkdir -p /home/renderer/src \
  ...
  && git -C openstreetmap-carto-vizall-light checkout vizall.light.0.6 \
...
# Configure renderd
COPY renderd.conf /home/renderd.conf
RUN cat /home/renderd.conf > /usr/local/etc/renderd.conf
